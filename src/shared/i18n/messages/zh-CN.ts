export const zhCN = {
  ...
  userSettings: {
    ...
    sekaiData: {
      title: "Sekai 数据缓存",
      description: "管理分区服 master data 与 music metas 的本地 IndexedDB 缓存。Master 会读取 current_version.json 并按区服选择 dataVersion/cdnVersion；music metas 会用 HEAD 校验 ETag、Last-Modified 与 Content-Length 后再带参数下载。基础更新只拉取选择器所需表，组卡更新会补齐 wasm 推荐所需表，同区任务会合并排队。",
      region: "区服",
      ...
      clear: "清理",
      queueTitle: "更新队列",
      queueDescription: "后台会串行处理同一区服的数据检查、下载和写入，已有缓存可继续使用，更新完成后再切换显示状态。",
      queueEmpty: "暂无更新任务。",
      status: {
        idle: "等待更新",
        loading: "加载中",
  ...
  deckRecommend: {
    title: "组卡推荐",
    description: "选择账号、数据区服、模式与歌曲参数，在浏览器中用 wasm 执行推荐。",
    select: {
      loading: "正在准备数据...",
    },
    form: {
    ...
    result: {
      ...
      elapsed: "wasm 合计用时 {ms} ms",
      totalElapsed: "推荐总耗时 {ms} ms",
      dataElapsed: "获取数据",
      engineDataElapsed: "wasm 数据准备",
    ...
    dataStatus: {
      title: "Master 数据",
      description: "显示当前区服的 master 与 music metas 缓存。Master 检查版本文件，metas 先做 HEAD 校验；已有缓存会先用于页面，后台队列完成后更新版本和时间。",
      region: "区服",
      masterVersion: "展示版本",
      fetchVersion: "拉取版本",
      updatedAt: "更新时间",
    ...
    dataQueue: {
      title: "更新队列",
      description: "同一区服的数据请求会合并排队，避免多个选择器同时刷新造成卡顿；队列会显示正在检查、下载或写入的文件。",
      empty: "暂无更新任务。",
      phases: {
        queued: "排队中",
        checking: "检查版本",
