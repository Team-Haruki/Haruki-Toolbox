import { C as e, S as t, T as n, X as r, Z as i, _ as a, c as o, d as s, f as c, i as l, it as u, l as d, p as f, rt as p, t as m, u as h, v as g, w as _, y as v } from "./animationPlaybackRuntime-CWE76hAs.js";
...
var it = Math.PI / 180, at = [
	...
	static fromPjskRuntimeExtension(t, n) {
		...
		let i = ft(n), a = [];
		for (let e of r) {
			let t = e.GameObject ?? e.gameObject ?? null, n = e.ReferenceBone ?? e.referenceBone ?? null, r = ht(i, yt(t?.TransformPath ?? t?.transformPath), yt(t?.Name ?? t?.name)), o = ht(i, yt(n?.TransformPath ?? n?.transformPath), yt(n?.Name ?? n?.name));
			if (!r || !o) continue;
			let s = at[M(e.RotationOrder ?? e.rotationOrder, 4)] ?? "ZXY";
			a.push({
				node: r,
				referenceNode: o,
				coefficient: M(e.Coefficient ?? e.coefficient, 1),
				defaultQuaternion: lt(e),
				axisX: bt(e.AxisX ?? e.axisX, !0),
				axisY: bt(e.AxisY ?? e.axisY, !0),
				axisZ: bt(e.AxisZ ?? e.axisZ, !0),
				order: s
			});
		}
...
function lt(e) {
	let t = j(e.DefaultEulerAngles ?? e.defaultEulerAngles) ?? {}, n = M(t.X ?? t.x, 0), r = M(t.Y ?? t.y, 0), i = M(t.Z ?? t.z, 0);
...
var xt = new y.Vector3(.04835, .48222, .07241), St = new y.Vector3(-.08532, .12848, 1.93551), Ct = 35, wt = .46, Tt = 2.74, Et = -.0245, Dt = .765, N = {
	zoomDuration: .35,
...
function Ot(e) {
	let t = xt.clone().multiplyScalar(e);
	return {
		target: t,
		position: t.clone().add(St.clone().multiplyScalar(e)),
		fov: Ct,
		costumeShopState: null
	};
}
function kt(e, t = 0, n = 1.6) {
	let r = Number.isFinite(t) ? t : 0;
	if (e === "legacy-cloud") {
		let e = new y.Vector3(0, wt, 0), t = new y.Vector3(0, 0, Tt).applyAxisAngle(new y.Vector3(0, 1, 0), y.MathUtils.degToRad(r));
		return {
			target: e,
	...
	let i = e === "official-default" ? {
		...
		cameraRootYawDegrees: r,
		zoomValue: N.zoomDuration,
		zoomMoveValue: 0
	}, a = y.MathUtils.clamp(i.zoomValue, 0, N.zoomDuration), o = N.zoomDuration > 0 ? a / N.zoomDuration : 0, s = y.MathUtils.lerp(N.bottomLowerLimitPosition, N.bottomUpperLimitPosition, o), c = y.MathUtils.lerp(N.topLowerLimitPosition, N.topUpperLimitPosition, o), l = y.MathUtils.clamp(i.zoomMoveValue, 0, 1), u = e === "full-body" ? Dt : y.MathUtils.lerp(s, c, l), d = y.MathUtils.lerp(N.nearZ, N.farZ, o), f = y.MathUtils.degToRad(i.cameraRootYawDegrees), p = new y.Vector3(0, u, d);
	return {
...
function At(e, t, n, r) {
	let i = t.clone().sub(e).normalize(), a = new y.Vector3().crossVectors(i, new y.Vector3(0, 1, 0)).normalize().multiplyScalar(Et * n * r);
	return {
...
function Nt(e) {
	let t = jt, n = Number.isFinite(e.zoom) ? e.zoom : Mt.zoom, r = Number.isFinite(e.heightOffset) ? e.heightOffset : Mt.heightOffset;
	return {
		zoom: y.MathUtils.clamp(n, t.minZoom, t.maxZoom),
		heightOffset: y.MathUtils.clamp(r, t.minHeightOffset, t.maxHeightOffset)
	};
}
function Pt(e, t) {
	let { zoom: n, heightOffset: r } = Nt(t), i = e.target.clone();
	i.y += r;
	let a = e.position.clone().sub(e.target).divideScalar(n);
	return {
		target: i,
		position: i.clone().add(a)
	};
}
//#endregion
//#region src/engine/captureBackground.ts
function Ft(e, t) {
	let n = Lt(e, t), r = new y.CanvasTexture(n);
...
function Lt(e, t) {
	...
	let o = It(e * 73856093 ^ t * 19349663), s = [
		[
			166,
			236,
			255
...
var Rt = ["Left_Toe", "Right_Toe"], zt = .015, Bt = .01, Vt = {
	width: .72,
	height: 1.06,
	opacity: .28,
	crossSize: .46,
	...
	update(e) {
		...
		for (let [n, r] of this.pairs.entries()) {
			...
			r.crossAnchor.position.set(i.x, this.settings.floorY + zt, i.z), r.crossMaterial.opacity = this.settings.crossOpacity * d;
		}
	}
	getDebugSnapshot(e) {
		let t = this.pairs[0];
		...
		return {
			...
			targetPosition: P(r),
			targetPositions: this.pairs.map((e) => P(e.targetWorldPosition)),
			directional: {
				position: P(t.directionalAnchor.position),
				forward: P(n),
				scale: P(new y.Vector3(this.settings.width, 1, this.settings.height)),
				opacity: Number(t.directionalMaterial.opacity.toFixed(4)),
				alpha: Number(t.directionalAlpha.toFixed(4))
			},
			cross: {
				position: P(t.crossAnchor.position),
				scale: P(new y.Vector3(this.settings.crossSize, 1, this.settings.crossSize)),
				opacity: Number(t.crossMaterial.opacity.toFixed(4))
			},
			pairs: this.pairs.map((e) => ({
				targetPosition: P(e.targetWorldPosition),
...
function Jt(e) {
	return y.MathUtils.clamp(e || Gt, .5, 2);
}
//#endregion
...
function Qt(e) {
	let t = Yt(Yt(e).motionPackage ?? Yt(e).MotionPackage);
...
var $t = class {
	...
	bind(e, t) {
		...
		return this.bindings.length = 0, e.traverse((e) => {
			if (e.userData.pjskEyeThroughHairOverlay || e.userData.pjskEyeThroughHairStencilPrepass || !Xt(e)) return;
		...
		if (!(!this.enabled || !this.clip)) for (let e of this.bindings) {
			...
			if (t) {
				...
				for (let n of this.clip.curves) {
					...
					r !== void 0 && (t[r] = Zt(n.keyframes, this.time) / 100);
				}
			}
		}
	}
...
function I(e, t) {
	let n = e?.floatProperties?.find((e) => F(e.name, t));
	if (Number.isFinite(n?.value)) return n.value;
	let r = e?.intProperties?.find((e) => F(e.name, t));
	return Number.isFinite(r?.value) ? r.value : null;
}
function en(e, t, n) {
	let r = I(e, t);
	return r === null ? n && (e?.validKeywords?.some((e) => F(e, n)) || e?.invalidKeywords?.some((e) => F(e, n))) ? !0 : null : r > .5;
}
function L(e, t) {
	let n = e?.colorProperties?.find((e) => F(e.name, t));
...
function an(e, t) {
	...
	return Be(n, {
		...
		rimDirection: e.uniforms.uRimDirection?.value.clone() ?? z(),
...
async function B(e, t, n = y.SRGBColorSpace) {
	if (!t) return null;
	let r = `${n}\u0000${t}`, i = on.get(e);
	i || (i = /* @__PURE__ */ new Map(), on.set(e, i));
	let a = i.get(r);
	if (a) return a;
...
function ln(e, t, n) {
	if (!t) return;
	let r = (e, t) => I(n, e) ?? t, i = (e, t, r) => en(n, e, t) ?? r, a = (e) => Math.round(y.MathUtils.clamp(e, 0, 1) * 255).toString(16).padStart(2, "0"), o = (e, t) => {
		let r = L(n, e);
	...
	return {
		...
		useOutlineSecondNormal: I(n, "_UseOutlineSecondNormal") ?? (en(n, "_UseOutlineSecondNormal", "_OUTLINE_SECOND_NORMAL") === !0 ? 1 : t.useOutlineSecondNormal),
		sekaiShadowThreshold: I(n, "_SekaiShadowThreshold") ?? t.sekaiShadowThreshold,
		...
		rangeLimit: I(n, "_RangeLimit") ?? t.rangeLimit,
...
function un(e, t) {
	...
	let a = en(t, "_UseAlphaClip", "_ALPHATEST_ON");
	a !== null && n.uAlphaCutoff && (n.uAlphaCutoff.value = a ? y.MathUtils.clamp(I(t, "_Cutoff") ?? .5, 0, 1) : 0);
}
function dn(e) {
...
function fn(e, t, n, r) {
	if (e.side = y.FrontSide, e.transparent = !0, e.stencilWrite = !0, e.stencilRef = t, e.stencilFunc = y.EqualStencilFunc, e.stencilFuncMask = t, e.stencilWriteMask = t, e.stencilFail = y.KeepStencilOp, e.stencilZFail = y.KeepStencilOp, e.stencilZPass = y.KeepStencilOp, e.depthTest = !0, e.depthWrite = !1, e.depthFunc = y.AlwaysDepth, e.blending = y.CustomBlending, e.blendSrc = y.SrcAlphaFactor, e.blendDst = y.OneMinusSrcAlphaFactor, e.blendEquation = y.AddEquation, e.blendSrcAlpha = y.ZeroFactor, e.blendDstAlpha = y.OneFactor, e.blendEquationAlpha = y.AddEquation, e.polygonOffset = !1, n) {
		let t = rn[n], i = (e, t) => I(r, e) ?? L(r, e)?.r ?? t, a = {
...
function _n(e, t, n, r) {
	...
	if (o.name = `${e.name}_${r}`, o.position.copy(e.position), o.quaternion.copy(e.quaternion), o.scale.copy(e.scale), o.matrix.copy(e.matrix), o.matrixAutoUpdate = e.matrixAutoUpdate, o.matrixWorldAutoUpdate = e.matrixWorldAutoUpdate, o.layers.mask = e.layers.mask, o.visible = e.visible, o.renderOrder = Math.min(...n.map((e) => H(typeof e.userData.pjskMaterialKind == "string" ? e.userData.pjskMaterialKind : ""))), o.frustumCulled = e.frustumCulled, o.castShadow = !1, o.receiveShadow = !1, o.morphTargetDictionary = e.morphTargetDictionary, o.morphTargetInfluences = e.morphTargetInfluences, gn(o, n), o.isSkinnedMesh && a.isSkinnedMesh) {
		let e = o;
...
function vn(e, t, n) {
	let r = _n(e, t, n, "through_hair_overlay");
	if (!r) return null;
...
async function yn({ root: e, bodyAsset: t, headAsset: n, textureLoader: r, template: i, bodyDebugMode: a, debug: o = [] }) {
	let s = await Promise.all(t.bodyMaterials.map(async (e) => {
		...
		let [o, s, c] = await Promise.all([
			B(r, e.mainTex),
			B(r, e.shadowTex),
			B(r, e.valueTex, y.NoColorSpace)
		]);
		R(o, e.rawMaterial, "_MainTex"), R(s, e.rawMaterial, "_ShadowTex"), R(c, e.rawMaterial, "_ValueTex");
		let l = ln(e.materialKind, e.lighting, e.rawMaterial), u = an(i, {
			mainTex: o,
			shadowTex: s,
			valueTex: c,
		...
		return un(u, e.rawMaterial), dn(u), u.userData.pjskLighting = l, u.userData.pjskRawMaterial = e.rawMaterial, u.userData.pjskMaterialKind = e.materialKind, u.userData.pjskMaterialKey = e.materialKey, u.userData.pjskMaterialSlotIndex = e.slotIndex, {
			key: e.materialKey,
			meshKey: V(e.meshName),
	...
	return e.traverse((e) => {
		let t = e;
		if (!t.isMesh) return;
		let n = Array.isArray(t.material) ? t.material : [t.material], r = s.filter((e) => e.meshKey === V(t.name));
		...
		let i = n.map((e) => {
			...
			let a = sn(e);
			cn(i.material, a), t.userData.pjskMaterialKind = i.materialKind;
			let s = !1;
...
function Cn(e, t, n, r, i = !0) {
	...
	e.push({
		...
		renderOrder: H(typeof r.userData.pjskMaterialKind == "string" ? r.userData.pjskMaterialKind : "")
	});
}
function wn(e, t) {
	let n = e.clone();
...
function kn(e, t, n, r, i, a, o, s) {
	...
	if (f === "eye") {
		...
		n.side = y.FrontSide, mn(n, U), n.userData.pjskMaterialKind = "eye_stencil_prepass";
		let r = A(c, "eye", o?.baseTiling, {
			...t,
			strictAlpha: !0
		});
		r.side = y.FrontSide, fn(r, U, "eye", e.rawMaterial), r.userData.pjskMaterialKind = "eye_through_hair", p.userData.pjskOverlayMaterial = r, p.userData.pjskStencilPrepassMaterial = n;
	} else if (f === "eyelight") {
		let t = On(o, s);
		...
		n.side = y.FrontSide, fn(n, U, "eyelight", e.rawMaterial), n.userData.pjskMaterialKind = "eyelight_through_hair", p.userData.pjskOverlayMaterial = n;
	} else if (f === "eyelash" || f === "eyebrow") {
		p = A(c, "alpha", null, { vertexBViewOffset: .015 }), h = An(t, n, r.face, s), p.side = y.FrontSide;
		let i = A(c, "alpha", null, { strictAlpha: !0 });
		i.side = y.FrontSide, mn(i, U), i.userData.pjskMaterialKind = `${f}_stencil_prepass`;
		let a = A(c, "alpha", null, { strictAlpha: !0 });
		a.side = y.FrontSide, fn(a, U, f, e.rawMaterial), a.userData.pjskMaterialKind = `${f}_through_hair`, p.userData.pjskOverlayMaterial = a, p.userData.pjskStencilPrepassMaterial = i;
	} else if (f === "hair") p = an(r.hair, {
		mainTex: c,
		shadowTex: l,
		valueTex: u,
	...
	else if (f === "accessory" || f === "body") p = an(r.body, {
		mainTex: c,
		shadowTex: l,
		valueTex: u,
		...
		lighting: s,
		bodyDebugMode: i.bodyDebugMode,
		alphaCutoff: f === "accessory" ? xn : 0
	}), dn(p);
	else {
		let e = An(t, n, r.face, s);
		e.uniforms.uFaceDebugMode && (e.uniforms.uFaceDebugMode.value = i.faceDebugMode), e.side = y.FrontSide, dn(e), p = e;
	}
	return {
		material: p,
		topLayerMaterial: m,
...
function jn(e, t, n, r, i, a, o) {
	...
	Mn(c, l);
	let u = Nn(c, l);
	e.renderOrder = H(c.materialKind), e.userData.pjskMaterialKind = c.materialKind;
	let d = En(e), f = c.material.userData.pjskLighting, p = c.materialKind === "face_sdf" && !!c.faceShadowTex && f?.useFaceSdf !== !1;
	return Fn(c.material, { faceSdfEnabled: i }, p, d), o[n] = c, Sn(a, e, t, c, u, p, d), c.material;
}
function Mn(e, t) {
	cn(e.material, t);
	for (let n of [
...
function Nn(e, t) {
	if (e.material instanceof y.ShaderMaterial && !e.material.uniforms.uMainTex.value && t) {
		...
		for (let n of [
			...
			e.stencilPrepassMaterial,
			e.topLayerMaterial
		]) n instanceof y.ShaderMaterial && Pn(n, t);
...
function In(e, t, n, r, i) {
	...
	for (let c of e) {
		...
		Ln(a, c, e?.topLayerMaterial ?? null), Ln(o, c, e?.overlayMaterial ?? null), Ln(s, c, e?.stencilPrepassMaterial ?? null);
		let l = n[c.materialIndex]?.name ?? "";
		e?.topLayerMaterial && Cn(i, r, l, e.topLayerMaterial), e?.stencilPrepassMaterial && Cn(i, r, l, e.stencilPrepassMaterial, !1), e?.overlayMaterial && Cn(i, r, l, e.overlayMaterial);
	}
	return {
		top: a,
		overlay: o,
...
async function Bn({ root: e, headAsset: t, textureLoader: n, templates: r, view: i, hair: a, eyeController: o, debug: s = [] }) {
	let c = [], l = [], u = [], d = await Promise.all(t.faceMaterials.map(async (e) => {
		let [s, c, l, u] = await Promise.all([
			B(n, e.mainTex),
			B(n, e.shadowTex),
			B(n, e.valueTex, y.NoColorSpace),
			B(n, e.faceShadowTex, y.NoColorSpace)
		]);
		if (R(s, e.rawMaterial, "_MainTex"), R(c, e.rawMaterial, "_ShadowTex"), R(l, e.rawMaterial, "_ValueTex"), R(u, e.rawMaterial, "_FaceShadowTex"), !e.materialKind) throw Error(`Head material ${e.materialName ?? e.materialKey} is missing materialKind.`);
		let d = e.materialKind, f = !!e.isAccessory || d === "accessory", p = ln(d, e.lighting, e.rawMaterial), { material: m, topLayerMaterial: h, outlineSourceMaterial: g } = kn(e, {
			mainTex: s,
			shadowTex: c,
			valueTex: l,
			faceShadowTex: u
		}, t, r, i, a, o, p);
		return un(m, e.rawMaterial), g && (un(g, e.rawMaterial), m.userData.pjskOutlineSourceMaterial = g), m.userData.pjskLighting = p, m.userData.pjskRawMaterial = e.rawMaterial, m.userData.pjskMaterialKind = d, m.userData.pjskIsAccessory = f, m.userData.pjskMaterialKey = e.materialKey, m.userData.pjskMaterialSlotIndex = e.slotIndex, h && (h.userData.pjskLighting = p, h.userData.pjskRawMaterial = e.rawMaterial, h.userData.pjskMaterialKind = d, h.userData.pjskIsAccessory = f, h.userData.pjskMaterialKey = e.materialKey, h.userData.pjskMaterialSlotIndex = e.slotIndex), {
			key: e.materialKey,
			meshKey: V(e.meshName),
			materialKey: e.materialKey,
			materialKind: d,
	...
	e.traverse((e) => {
		...
		let n = Array.isArray(t.material) ? t.material : [t.material], r = V(t.name), a = d.filter((e) => e.meshKey === r);
		if (a.length === 0) return;
		let o = [], f = n.map((e, n) => jn(t, e, n, a, i.faceSdfEnabled, s, o)), p = o.reduce((e, t) => t ? Math.min(e, H(t.materialKind)) : e, Infinity);
		Number.isFinite(p) && (t.renderOrder = p);
		let m = In(t.geometry.groups.length > 0 ? t.geometry.groups.map((e) => ({
			start: e.start,
			count: e.count,
			materialIndex: e.materialIndex ?? 0
		})) : [{
		...
		Tn(n, f), gn(t, f), t.material = Array.isArray(t.material) || f.length > 1 ? f : f[0], t.castShadow = !1, t.receiveShadow = !1, Rn(t, m.stencil, "stencil_prepass", l), Rn(t, m.overlay, "overlay", c), zn(t, m.top, u);
	});
...
var Vn = {
	pushBoundMaterialDebug: Sn,
	pushLayerMaterialDebug: Cn,
	cloneFaceShaderMaterial: wn,
	disposeReplacedMaterials: Tn,
	hasFaceSdfUv1Attribute: En,
	createEyeLayerOptions: Dn,
	createHighlightLayerOptions: On,
	createHeadSlotMaterials: kn,
	createHeadFaceMaterial: An,
	syncHeadSlotTextures: Mn,
	applyOriginalHeadMap: Nn,
	applyOriginalMapToShader: Pn,
	updateHeadFaceSdfState: Fn,
	collectHeadLayerPasses: In,
	addHeadLayerPass: Ln,
	queueThroughHairPasses: Rn,
	queueTopLayerPasses: zn
}, W = {
	widthMin: 4e-4,
	widthMax: .0095,
	distanceNear: .45,
...
function Un(e) {
	let t = Number.isFinite(e) ? e : 25, n = Hn, r;
...
function Wn() {
	return new y.Vector2(W.widthMin, W.widthMax);
}
function Gn(e) {
...
function Jn(e, t, n) {
	...
	r && (t && typeof t == "object" && "r" in t && "g" in t && "b" in t ? r.color.setRGB(t.r, t.g, t.b) : D(r.color, t ?? new y.Color().setRGB(G.color.r, G.color.g, G.color.b)), r.blending = y.MathUtils.clamp(n ?? G.blending, 0, 1));
}
function Yn(e, t, n, r) {
	let i = new y.Vector3(W.distanceNear, 1 / (W.distanceFar - W.distanceNear), Un(25)), a = {
		color: new y.Color().setRGB(G.color.r, G.color.g, G.color.b),
		blending: G.blending
	}, o = I(n, "_OutlineOffset") ?? 0, s = e.clone();
	...
	return s.uniforms = {
		...
		n instanceof y.PerspectiveCamera && (i.z = Un(n.fov));
	}, s;
}
function Xn(e, t, n = !1, r = null, i = null) {
	if (i instanceof y.ShaderMaterial && /vec3 outputColor\s*\(\s*vec3 color\s*\)/.test(i.fragmentShader)) return Yn(i, e, t, n);
	let a = L(t, "_OutlineColor") ?? Kn, o = tn(t, "_MainTex"), s = new y.Vector4(o?.scaleX ?? 1, o?.scaleY ?? 1, o?.offsetX ?? 0, o?.offsetY ?? 0), c = (I(t, "_UseAlphaClip") ?? 0) > .5, l = y.MathUtils.clamp(I(t, "_Cutoff") ?? .5, 0, 1), u = I(t, "_OutlineOffset") ?? 0, d = new y.Color().setRGB(a.r, a.g, a.b), f = {
		color: new y.Color().setRGB(G.color.r, G.color.g, G.color.b),
		blending: G.blending
		...
		vertexColors: !1,
		alphaTest: c ? l : 0
	}), m = new y.Vector3(W.distanceNear, 1 / (W.distanceFar - W.distanceNear), Un(25));
	return p.name = "pjsk_shell_outline", p.userData.pjskOutlineController = f, p.onBeforeCompile = (t) => {
		t.uniforms.uSekaiOutlineWidth = { value: Wn() }, t.uniforms.uSekaiOutlineFactor = { value: m }, t.uniforms.uSekaiOutlineOffset = { value: u }, t.uniforms.uSekaiMainTexST = { value: s }, t.uniforms.uSekaiCharacterOutlineColor = { value: f.color }, t.uniforms.uSekaiCharacterOutlineBlending = { get value() {
			return f.blending;
		...
		n instanceof y.PerspectiveCamera && (m.z = Un(n.fov));
	}, p;
}
//#endregion
...
function Qn(e, t) {
	...
	n instanceof y.Object3D && (e.layers.mask = n.layers.mask), e.visible = a && !t.outlineOnly && !t.eyelightOnly && !t.noEyeThroughHair && dr(r, t.mode) && fr(r, i, t.mode) && t.faceLayersVisible && (!t.noEyelight || r !== "eyelight"), e.userData.pjskEyeThroughHairBaseVisible = e.visible;
}
function $n(e, t) {
	...
	let r = ur(n);
	e.visible = !t.eyeThroughHairOnly && t.outlineVisible && !pr(n, t.mode) && (!t.noEyelight || n !== "eyelight") && (!r || t.faceLayersVisible);
}
function er(e, t) {
	let n = Array.isArray(e.material) ? e.material : [e.material], r = tr(n, t.faceSdfEnabled);
...
function pr(e, t) {
	switch (t) {
		...
		case "no_face_layers":
		case "no_face_outline": return ur(e);
		default: return !1;
	}
}
var mr = class {
	...
	controllerOutlineColor = new y.Color().setRGB(G.color.r, G.color.g, G.color.b);
	...
	getBindingView() {
		return {
			bodyDebugMode: sr(this.bodyDebugMode),
			faceDebugMode: cr(this.faceSdfDebugMode),
	...
	setHairShadowMode(e) {
		this.hairShadowMode = ir(e), this.options.debug.hairShadowMode = this.hairShadowMode, this.applyHairShadowMode();
	}
	...
	applyCharacterSkinColors() {
		...
		for (let t of this.debugEntries) for (let n of t) rr(n, e);
	}
	shouldEnableFaceSdf() {
	...
	applyFaceSdfDebug() {
		let e = cr(this.faceSdfDebugMode);
	...
	applyBodyDebug() {
		let e = sr(this.bodyDebugMode), t = (t) => {
	...
	applyRenderIsolationMode() {
		...
		for (let e of this.slots) e.traverse((e) => Zn(e, n));
		for (let t of this.debugEntries) for (let n of t) (n.shaderFaceSdfEnabled !== void 0 || n.resolvedKind === "face_sdf") && (n.shaderFaceSdfEnabled = e && n.faceSdfCapable === !0 ? 1 : 0);
	}
	...
	updatePreviewLight(e, t, n, r, i, a = z()) {
	...
	updateLoadedMaterialLight(e, t, n = z()) {
	...
	updateControllerOutline(e) {
		this.controllerOutlineColor = e.color ? D(new y.Color(), e.color) : new y.Color().setRGB(G.color.r, G.color.g, G.color.b), this.controllerOutlineBlending = y.MathUtils.clamp(e.blending ?? G.blending, 0, 1);
		for (let e of this.slots) e.traverse((e) => {
			let t = e;
	...
	applyOutlineMaterial(e) {
		Jn(e, this.controllerOutlineColor, this.controllerOutlineBlending);
	}
}, hr = {
	applyRenderIsolationToNode: Zn,
	applyEyeThroughHairIsolation: Qn,
	applyOutlineIsolation: $n,
	applyBaseMeshIsolation: er,
	inspectFaceLayers: tr,
	applySkinColors: nr,
	applyDebugEntrySkinColors: rr,
	normalizeHairShadowMode: ir,
	isFaceLayerMaterialKind: lr,
	isFaceOrFaceLayerMaterialKind: ur,
	isEyeThroughHairSourceAllowed: dr,
	isEyeThroughHairPassAllowed: fr,
	isOutlineHiddenByIsolation: pr
}, gr = class {
	constructor(e = 4) {
...
function xr(e) {
	...
	let n = new vr(), r = 17 * Uint32Array.BYTES_PER_ELEMENT, i = new yr(e, K.length, r, !0);
	...
	let o = i._nextUint32(), s = i._nextUint32(), c = i._nextUint32(), l = i._nextUint32(), u = i._nextUint64(), d = i._nextUint64(), f = new yr(e, K.length + r, 3 * a * 8, !0);
	...
	let g = new yr(e, c, l, !0);
	for (; g._offset < l;) {
		let e = g._nextUint32(), t = g._scan(e), r = br(t);
		if (n.keyValue[r] = g._nextUint8Array(e - t.byteLength - 1), r.match(/^ktx/i)) {
			let e = br(n.keyValue[r]);
			n.keyValue[r] = e.substring(0, e.lastIndexOf("\0"));
		}
		g._skip(e % 4 ? 4 - e % 4 : 0);
	}
	if (d <= 0) return n;
	let _ = new yr(e, u, d, !0), v = _._nextUint16(), y = _._nextUint16(), ee = _._nextUint32(), te = _._nextUint32(), ne = _._nextUint32(), re = _._nextUint32(), ie = [];
...
var Sr, q, Cr, wr = { env: { emscripten_notify_memory_growth: function(e) {
	...
	init() {
		if (!this.transcoderPending) {
			...
			}), Ar > 0 && console.warn("THREE.KTX2Loader: Multiple active KTX2 loaders may cause performance issues. Use a single KTX2Loader instance, or call .dispose() on old instances."), Ar++;
		}
	...
	parse(e, t, n) {
		if (this.workerConfig === null) throw Error("THREE.KTX2Loader: Missing initialization with `.detectSupport( renderer )`.");
		if (kr.has(e)) return kr.get(e).promise.then(t).catch(n);
	...
	_createTextureFrom(e, t) {
		...
		return u.minFilter = i[0].mipmaps.length === 1 ? oe : se, u.magFilter = oe, u.generateMipmaps = !1, u.needsUpdate = !0, u.colorSpace = Ir(t), u.premultiplyAlpha = !!(l & 1), u;
	}
	async _createTexture(e, t = {}) {
		let n = xr(new Uint8Array(e)), r = n.vkFormat === 1000066e3 && n.dataFormatDescriptor[0].colorModel === 167;
		if (!(n.vkFormat === 0 || r && !this.workerConfig.astcHDRSupported)) return Fr(n);
		let i = t, a = this.init().then(() => this.workerPool.postMessage({
			type: "transcode",
			buffer: e,
			taskConfig: i
		}, [e])).then((e) => this._createTextureFrom(e.data, n));
		return kr.set(e, { promise: a }), a;
	}
	dispose() {
		this.workerPool.dispose(), this.workerSourceURL && URL.revokeObjectURL(this.workerSourceURL), Ar--;
	}
};
J.BasisFormat = {
	ETC1S: 0,
...
async function Fr(e) {
	let { vkFormat: t } = e;
	if (Nr[t] === void 0) throw Error("THREE.KTX2Loader: Unsupported vkFormat.");
	let n;
	e.supercompressionScheme === 2 && (jr ||= new Promise(async (e) => {
		let t = new Tr();
		await t.init(), e(t);
	}), n = await jr);
	let r = [];
	for (let i = 0; i < e.levels.length; i++) {
		...
		let u;
		u = Pr[t] === b ? new Float32Array(l.buffer, l.byteOffset, l.byteLength / Float32Array.BYTES_PER_ELEMENT) : Pr[t] === x ? new Uint16Array(l.buffer, l.byteOffset, l.byteLength / Uint16Array.BYTES_PER_ELEMENT) : l, r.push({
			data: u,
			width: a,
			height: o,
			depth: s
	...
	let i;
	if (Mr.has(Nr[t])) i = e.pixelDepth === 0 ? new ie(r[0].data, e.pixelWidth, e.pixelHeight) : new re(r[0].data, e.pixelWidth, e.pixelHeight, e.pixelDepth);
	else {
	...
	return i.mipmaps = r, i.type = Pr[t], i.format = Nr[t], i.colorSpace = Ir(e), i.needsUpdate = !0, Promise.resolve(i);
}
function Ir(e) {
	let t = e.dataFormatDescriptor[0];
	return t.colorPrimaries === 1 ? t.transferFunction === 2 ? Ee : ce : t.colorPrimaries === 10 ? t.transferFunction === 2 ? Dr : Or : (t.colorPrimaries === 0 || console.warn(`THREE.KTX2Loader: Unsupported color primaries, "${t.colorPrimaries}"`), ue);
}
//#endregion
...
var Br = 1, Vr = new y.Vector3(De.x, De.y, De.z), Hr = new y.Vector3(Oe.x, Oe.y, Oe.z).normalize(), Ur = new y.Vector3(0, 1, 0), Wr = !0, Gr = 0, Kr = 1e-5, qr = t("up");
function Jr(e) {
	return e && typeof e == "object" ? e : {};
}
function Y(e) {
...
function Xr(e, t, n) {
	let r = Jr(e), i = Jr(r.pjskSpringBone ?? r.PjskSpringBone), a = Jr(i.raw ?? i.Raw), o = Yr(a.body ?? a.Body), s = Yr(a.head ?? a.Head), c = !!(a.body ?? a.Body ?? a.head ?? a.Head);
	return {
		present: c,
		runtimePresent: !!n,
		active: !!n?.enabled,
...
function Zr(e, t, n = /* @__PURE__ */ new Set()) {
	if (!(!e || typeof e != "object" || n.has(e))) {
		...
		if (!(e instanceof y.Color || e instanceof y.Vector2 || e instanceof y.Vector3 || e instanceof y.Vector4 || e instanceof y.Matrix3 || e instanceof y.Matrix4 || ArrayBuffer.isView(e) || e instanceof ArrayBuffer)) {
			if (Array.isArray(e)) {
				for (let r of e) Zr(r, t, n);
				return;
			}
			for (let r of Object.values(e)) Zr(r, t, n);
		}
	}
}
function Qr(e, t = !0, n = /* @__PURE__ */ new Set(), r = /* @__PURE__ */ new Set()) {
...
function $r(e, t = /* @__PURE__ */ new Set()) {
	...
	e.traverse((e) => {
		let i = e;
		i.isMesh && (i.geometry && !i.userData.pjskOutlineShell && !n.has(i.geometry) && (i.geometry.dispose(), n.add(i.geometry)), i.material && Qr(i.material, !0, t, r));
	});
}
function ei(e, t = /* @__PURE__ */ new Set()) {
	for (let n of [...e.children]) $r(n, t), e.remove(n);
}
function ti(e) {
...
function ri(e) {
	...
	return (V(e.name) === "acc" || i.includes("/acc") || r.some((e) => e.includes("_acc") || e.startsWith("mtl_acc"))) && t.add("accessory"), [...t];
}
function ii(e) {
	return e.find((e) => !ni(e)) ?? e[0] ?? null;
}
function ai(e) {
	return e.length > 0 && e.every(ni);
}
function oi(e) {
...
function X(e, t, n, r = 0, i = 1) {
	...
	return a <= Kr ? e.set(r, i) : e.set(t / a, n / a);
}
function si(e, t) {
	let n = Math.abs(t - e);
...
function ui(e) {
	...
	return Object.keys(r).length ? {
		...
		tintColor: ci(r.tintColor ?? r.TintColor),
		emissionColor: ci(r.emissionColor ?? r.EmissionColor),
		baseTiling: li(r.baseTiling ?? r.BaseTiling),
		highlightTiling: li(r.highlightTiling ?? r.HighlightTiling)
	} : null;
}
function di(e) {
...
var gi = class {
	...
	constructor(e, t) {
		...
		let i = n.canvas ?? n.container, a = this.ownsCanvas ? 320 : 1, o = Math.max(i.clientWidth, a), s = Math.max(i.clientHeight, a), c = Ot(this.characterModelScaleMeters);
		...
		let l = zr(o, s, window.devicePixelRatio);
		this.renderer.setPixelRatio(l), this.renderer.setSize(o, s, this.ownsCanvas), this.viewportWidth = o, this.viewportHeight = s, this.viewportPixelRatio = l, this.renderer.outputColorSpace = y.SRGBColorSpace, this.container && this.renderer.domElement.parentElement !== this.container && this.container.appendChild(this.renderer.domElement), this.updateCaptureBackgroundTexture(), this.cameraTarget.copy(c.target), n.controlsFactory ? (this.controls = n.controlsFactory({
			...
			baseColor: "#ffe4dc",
			warmColor: "#ffd4c8",
			lightDirection: Hr.clone(),
			lightIntensity: r.intensity,
			ambientIntensity: r.ambient,
			headDotDirectionalLight: this.headDotDirectionalLight,
			useFaceShadowLimiter: Wr,
			faceShadowLimitRange: Gr,
			shadowThreshold: r.shadowThreshold,
			shadowWeight: r.shadowWeight,
			useLambert: !0
		}), this.characterRoot = new y.Group(), this.bodySlot = new y.Group(), this.headSlot = new y.Group(), this.characterRoot.add(this.bodySlot), this.characterRoot.add(this.headSlot), this.characterLighting = new mr({
			...
			valueShadowInfluence: Br
		}), this.scene.add(this.characterRoot), this.projectedShadow = new Ht(), this.scene.add(this.projectedShadow.group), this.setPresentationMode(n.presentationMode ?? "interactive"), this.applyCameraPreset(n.cameraPreset ?? "default", n.cameraProfile), this.handleResize = this.handleResize.bind(this), this.manageResize && (window.addEventListener("resize", this.handleResize), this.handleResize()), this.autoRender && this.render();
	}
	...
	setViewYawDegrees(e) {
		this.cameraRootYawDegrees = Number.isFinite(e) ? e : 0, this.applyCostumeShopView();
	}
	setViewZoom(e) {
		this.viewFraming = Nt({
			...this.viewFraming,
	...
	applyCostumeShopView() {
		let e;
		if (this.currentCameraPreset === "capture") e = kt(this.currentCameraProfile ?? "full-body", this.cameraRootYawDegrees, this.masterCharacterHeightMeters);
		else {
			let t = Ot(this.characterModelScaleMeters), n = t.position.clone().sub(t.target).applyAxisAngle(Ur, y.MathUtils.degToRad(this.cameraRootYawDegrees));
			e = {
				target: t.target,
				position: t.target.clone().add(n),
				fov: t.fov
			};
		}
		let t = Pt(e, this.viewFraming);
		this.setCameraTarget(t.target), this.camera.position.copy(t.position), this.camera.fov = e.fov, this.camera.updateProjectionMatrix(), this.syncCameraTarget(), this.applyPreviewLightForCameraRoot(), this.updateShaderFaceBasis(), this.cameraDebugChangeCallback?.();
	}
	...
	getRuntimeDebugSnapshot() {
		return {
			...
			funit: di(this.currentRuntimeExtension),
			hairShadowOffset: $(this.currentHairOffset),
			hairShadowWorldPosition: $(this.hairHeadPosition),
	...
	getFaceLightDebugSnapshot() {
		...
		return {
			...
			costumeShopLightRotationDegrees: $(Vr),
			faceRightWorld: $(l),
			faceUpWorld: $(u),
			faceForwardWorld: $(c),
	...
	getCameraDebugSnapshot() {
		let e = this.camera.position, t = this.controls?.target ?? this.cameraTarget, n = e.clone().sub(t), r = new y.Spherical().setFromVector3(n), i = (this.currentCameraPreset === "capture" ? kt(this.currentCameraProfile ?? "full-body", this.cameraRootYawDegrees, this.masterCharacterHeightMeters) : null)?.costumeShopState ?? null;
		return {
	...
	getSpringBoneSnapshot(e) {
		return Xr(this.currentRuntimeExtension, !1, this.currentSpringRuntime?.getSnapshot(this.isSpringRuntimeEnabled(), e) ?? null);
	}
	...
	resolveProjectedShadowTargetWorldPositions() {
		let e = this.currentBodyAnimationRoot ?? this.characterRoot;
		return e.updateMatrixWorld(!0), Rt.map((t) => this.findNodeByImportedName(e, t)).filter((e) => e !== null).map((e) => e.getWorldPosition(new y.Vector3()));
	}
	getCanvas() {
	...
	setViewportSize(e, t) {
		let n = this.ownsCanvas ? 320 : 1, r = Math.max(Math.trunc(e) || 0, n), i = Math.max(Math.trunc(t) || 0, n), a = zr(r, i, window.devicePixelRatio);
	...
	async loadRuntimePackage(e) {
		...
		let a = r.combinedCharacter.bodyAsset.source.animationUrls?.[0], o = l(a ?? null), s = a && (o === "unity-json" || /body[_-]?motion/i.test(a.split(/[/?#]/)[0] ?? "")) ? a : null, c = Qt(r.combinedCharacter.runtimeExtension);
	...
	async loadRenderRecipeInternal(e, t) {
		...
		let i = g(e), a = mi(i.roleId), o = p(a.characterId, a.unit), s = this.currentLoadedRuntimePackage, c = s?.partSet?.baseUrl ?? null, l = s?.wardrobe?.getActiveRoleId() ?? null;
	...
	async applyCustomRoleDefaultMotion(e, t) {
		let n = e.bodyAsset.source.animationUrls?.[0], r = l(n ?? null), i = n && (r === "unity-json" || /body[_-]?motion/i.test(n.split(/[/?#]/)[0] ?? "")) ? n : null, a = Qt(e.runtimeExtension);
	...
	applyPreviewLightForCameraRoot() {
		let e = y.MathUtils.degToRad(this.cameraRootYawDegrees), t = new y.Vector3(this.previewLightBase.x, this.previewLightBase.y, this.previewLightBase.z).applyAxisAngle(Ur, e), n = z().applyAxisAngle(Ur, e);
	...
	getCameraRootFaceShadowLightDirection() {
		return Hr.clone().applyAxisAngle(Ur, y.MathUtils.degToRad(this.cameraRootYawDegrees));
	}
	updateGlobalShadowColor(e, t = 1) {
	...
	applyCostumeShopCharacterHeight(e) {
		let t = y.MathUtils.clamp(e || 1.6, .5, 2), n = qt(t), r = Math.abs(n - this.characterModelScaleMeters) >= 1e-4;
		...
		let i = this.currentCameraProfile === "legacy-cloud" ? kt("legacy-cloud", this.cameraRootYawDegrees, t) : Ot(n), a = i.position.clone().sub(i.target).applyAxisAngle(Ur, this.currentCameraProfile === "legacy-cloud" ? 0 : y.MathUtils.degToRad(this.cameraRootYawDegrees)), s = Pt({
			target: i.target,
			position: i.target.clone().add(a)
		}, this.viewFraming);
		this.setCameraTarget(s.target), this.camera.position.copy(s.position), this.syncCameraTarget();
	}
	applyCameraPreset(e, t = "full-body") {
		this.currentCameraPreset = e, this.currentCameraProfile = e === "capture" ? t : null, this.applyCostumeShopView();
	}
	shiftCameraRight(e = 1) {
		if (!Number.isFinite(e) || e === 0) return;
		let t = this.controls?.target ?? this.cameraTarget, n = At(this.camera.position, t, e, this.characterModelScaleMeters);
	...
	clearCharacterSlot(e) {
		ei(e, this.getPersistentCharacterMaterials());
	}
	...
	async loadCombinedCharacterAsset(e) {
		...
		return this.syncUnityPrefabSourceGraph(), await Promise.all([this.overrideBodyMaterials(t.root, e.bodyAsset), this.overrideHeadMaterials(t.root, e.headAsset, {
			eyeController: ui(e.runtimeExtension),
			hairController: fi(e.runtimeExtension)
	...
	installSekaiOutlineShell(e) {
		...
		let a = ii(t), o = Array.isArray(e.material) ? i : i[0], s = e instanceof y.SkinnedMesh ? new y.SkinnedMesh(e.geometry, o) : new y.Mesh(e.geometry, o);
	...
	createSekaiOutlineMaterials(e, t) {
		return t.map((t) => {
			...
			if (delete t.userData.pjskOutlineSourceMaterial, ni(t.userData.pjskMaterialKind)) {
				n?.dispose();
			...
			let i = (t.userData.pjskLighting?.useOutlineSecondNormal ?? 0) > .5, a = i && !!e.geometry.getAttribute("tangent") && !!e.geometry.getAttribute("uv1") && !!e.geometry.getAttribute("uv2"), o = Xn(!!e.geometry.getAttribute("color"), r, a, oi(t), n ?? t);
	...
	async overrideHeadMaterials(e, t, n = {}) {
		...
		}), this.currentHairHeadTransform ??= n.hairController?.headTransformName ? this.findNodeByImportedName(e, n.hairController.headTransformName) : null, await Bn({
			root: e,
			headAsset: t,
			textureLoader: this.textureLoader,
			templates: {
	...
	updateCaptureBackgroundTexture(e, t) {
		...
		this.captureBackgroundTexture?.dispose(), this.captureBackgroundTexture = Ft(r, i), this.scene.background = this.captureBackgroundTexture;
	}
	...
	updateShaderFaceBasis() {
		...
		e.getWorldQuaternion(this.tempQuaternion), e.getWorldPosition(this.faceHeadWorldPosition), this.headTransformUpWorld.copy(qr).applyQuaternion(this.tempQuaternion).normalize(), this.faceUpWorld.set(1, 0, 0).applyQuaternion(this.tempQuaternion).normalize(), this.faceForwardWorld.set(0, 0, 1).applyQuaternion(this.tempQuaternion).normalize(), this.faceRightWorld.crossVectors(this.faceUpWorld, this.faceForwardWorld).normalize(), this.faceUpWorld.crossVectors(this.faceForwardWorld, this.faceRightWorld).normalize();
...
var yi = _i, bi = vi;
//#endregion
export { Pt as $, sn as A, en as B, an as C, hn as D, mn as E, gn as F, Qt as G, I as H, cn as I, Ht as J, Kt as K, ln as L, z as M, B as N, _n as O, V as P, jt as Q, pn as R, yn as S, fn as T, tn as U, L as V, $t as W, Ft as X, Vt as Y, Mt as Z, G as _, gi as a, ze as at, Vn as b, Rr as c, Me as ct, hr as d, Ae as dt, Nt as et, cr as f, T as ft, Gn as g, Un as h, w as ht, bi as i, ot as it, H as j, vn as k, mr as l, Ne as lt, qn as m, De as mt, vi as n, Ot as nt, fi as o, Xe as ot, Xn as p, Oe as pt, qt as q, yi as r, At as rt, zr as s, A as st, _i as t, kt as tt, sr as u, Pe as ut, W as v, dn as w, un as x, Bn as y, R as z };
