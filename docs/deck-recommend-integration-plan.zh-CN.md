...
- 结果区：运行状态、获取数据耗时、wasm 数据准备耗时、顺序/并行推荐执行耗时、单算法耗时、卡组结果、来源算法标签、卡牌缩略图与指标。
...
- 已推进 6：结果区已展示 wasm 耗时、卡组指标、卡牌指标，并用 master `cards` 与 wasm 结果组装前端 DOM/CSS 卡牌缩略图。
- 仍待继续：更精细的结果排序/筛选、完整截图式导出、设置页缓存管理、PWA runtime cache、活动/歌曲高级筛选。
