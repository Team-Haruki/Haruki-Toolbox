...
- 组卡页面进入或刷新时，对当前数据服务器自动检查组卡所需完整 master 文件集和 music metas。
- 若已有可用缓存，立即使用缓存并后台检查更新。
- 若无缓存，页面显示当前 region 的准备状态。
- 点击开始推荐时只读取 IndexedDB 中已缓存的数据，不再触发 master/meta 版本检查。
...
Music metas 与 master data 一起按区服缓存，但保持独立 key，方便单独刷新和诊断。检查缓存命中时先对 metas 源做 `HEAD`，比较 `ETag` / `Last-Modified` / `Content-Length`；实际下载时带 query param 避免中间缓存返回旧内容。
...
- 稀有度行标题 icon：一二星使用 `static_images/card/rare_star_normal.png`，三四星使用 `static_images/card/rare_star_after_training.png`，生日使用 `static_images/card/rare_birthday.png`。
...
- 烤森画布 icon：`static_images/mysekai/icon/icon_canvas.png`。
...
- 配置区：账号、数据服务器、组卡模式、Live 类型、活动/角色/歌曲/难度、多选搜索算法、养成配置、高级参数。
- 顶部栏设置弹窗：按区服展示 master/meta 缓存版本、后台更新任务、刷新与清理操作。
- 结果区：运行状态、获取数据耗时、组卡引擎数据准备耗时、顺序/并行推荐执行耗时、单算法耗时、卡组结果、来源算法标签、卡牌缩略图与指标。
...
- DFS-GA 混合搜索算法（wasm `dfs_ga`）
- 遗传算法（wasm `ga`）
- 强化学习（wasm `rl`）

三个算法默认全部启用。搜索算法勾选状态与执行方式应持久化保存为本地偏好。执行方式由用户选择，默认顺序进行，复用单例 recommend worker；选择同时进行时按启用算法数量维护并预热并行 worker pool，为每个算法分配独立 recommend worker 并行计算。多个算法返回的卡组按卡牌集合去重，保留分数最高的卡组数据，并在结果卡组上用 tag 标记来源算法；同一套卡组可显示多个算法 tag。
...
- 活动类组卡仅当所选活动为 `world_bloom` 且 `worldBlooms.json` 存在 `worldBloomChapterType = game_character` 的章节时显示角色选择，并只展示这些章节角色；挑战组卡仍展示全角色列表。
...
行标题只显示图标，不展示“一星/二星”等文字：

```text
[star_icon × 稀有度等级]，生日使用生日 icon 单张
```
...
组卡页面进入时预热单例 recommend worker 与 wasm 实现的组卡引擎，降低首次点击开始推荐时的初始化等待；当后台检查确认当前数据服务器的组卡 master/meta 缓存已经 ready 且不在刷新中时，页面会继续把完整 master data 与 music metas 载入单例 worker。顺序执行算法时，共享 worker 已载入数据后每个算法请求只传 options 和数据 key，避免重复结构化克隆 master/meta。并行执行时，根据启用算法数量预热并行 worker pool，并在缓存 ready 后把 master/meta 预载入每个并行 worker；每个并行 worker 按 data key 去重预载，避免 watcher 同时触发时重复克隆数据。运行期间切换搜索算法或执行方式不立即缩减正在使用的 worker，待本次推荐完成后再同步并行 pool 大小。离开组卡页面时调用 engine `dispose()` 并 terminate 单例 worker 与并行 worker pool。
...
- 已完成 1-4：共享 Sekai 数据层、静态资源 URL resolver、活动/歌曲/角色 select、组卡表单与养成配置表。
- 已完成 5：接入 `haruki-sekai-deck-recommend-cpp` npm wasm 包，推荐在 worker 中执行，主线程通过后端 recommend-data API 获取用户 suite/mysekai 数据。
- 已推进 6：结果区已展示获取数据、组卡引擎数据准备、推荐执行与单算法耗时、卡组指标、卡牌指标，并用 master `cards` 与 wasm 结果组装前端 DOM/CSS 卡牌缩略图。
- 仍待继续：更精细的结果排序/筛选、完整截图式导出、顶部栏设置弹窗缓存管理细节、PWA runtime cache、活动/歌曲高级筛选。
